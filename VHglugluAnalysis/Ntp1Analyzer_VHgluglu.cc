#include "Ntp1Analyzer_VHgluglu.h"
...
Ntp1Analyzer_VHgluglu::Ntp1Analyzer_VHgluglu( const std::string& dataset, const std::string& flags, TTree* tree ) :
     Ntp1Analyzer( "VHgluglu", dataset, flags, tree ) {
...
void Ntp1Analyzer_VHgluglu::CreateOutputFile() {
...
//reducedTree_->Branch("HLT_DoubleMu3", &HLT_DoubleMu3_, "HLT_DoubleMu3_/O");
  ...
  reducedTree_->Branch("pdgIdPartMomJet", pdgIdPartMomJet_, "pdgIdPartMomJet_[nJets_]/I");
...
Ntp1Analyzer_VHgluglu::~Ntp1Analyzer_VHgluglu() {
...
void Ntp1Analyzer_VHgluglu::Loop()
{
...
if( DEBUG_VERBOSE_ ) std::cout << "entry n." << jentry << std::endl;
     ...
     // save trigger info:
     // double muon:
     passed_HLT_DoubleMu6_ = (isMC_) ? true : this->PassedHLT( jentry, "HLT_DoubleMu6");
     passed_HLT_DoubleMu7_ = (isMC_) ? true : this->PassedHLT( jentry, "HLT_DoubleMu7");
     passed_HLT_Mu13_Mu8_ = (isMC_) ? true : this->PassedHLT( jentry, "HLT_Mu13_Mu8");
     passed_HLT_Mu17_Mu8_ = (isMC_) ? true : this->PassedHLT( jentry, "HLT_Mu17_Mu8");
     passed_HLT_IsoMu17_ = (isMC_) ? true : this->PassedHLT( jentry, "HLT_IsoMu17");
     passed_HLT_IsoMu24_ = (isMC_) ? true : this->PassedHLT( jentry, "HLT_IsoMu24_v");
     passed_HLT_IsoMu24_eta2p1_ = (isMC_) ? true : this->PassedHLT( jentry, "HLT_IsoMu24_eta2p1_v");
     passed_HLT_Mu8_Jet40_ = (isMC_) ? true : this->PassedHLT( jentry, "HLT_Mu8_Jet40");
     passed_HLT_L2DoubleMu23_NoVertex_ = (isMC_) ? true : this->PassedHLT( jentry, "HLT_L2DoubleMu23_NoVertex");
     passed_HLT_L2DoubleMu30_NoVertex_ = (isMC_) ? true : this->PassedHLT( jentry, "HLT_L2DoubleMu30_NoVertex");
     passed_HLT_TripleMu5_ = (isMC_) ? true : this->PassedHLT( jentry, "HLT_TripleMu5");

     // double electron:
     passed_HLT_Ele17_CaloIdL_CaloIsoVL_Ele8_CaloIdL_CaloIsoVL_ = (isMC_) ? true : this->PassedHLT( jentry, "HLT_Ele17_CaloIdL_CaloIsoVL_Ele8_CaloIdL_CaloIsoVL");
     passed_HLT_Ele17_CaloIdT_CaloIsoVL_TrkIdVL_TrkIsoVL_Ele8_CaloIdT_CaloIsoVL_TrkIdVL_TrkIsoVL_ = (isMC_) ? true : this->PassedHLT( jentry, "HLT_Ele17_CaloIdT_CaloIsoVL_TrkIdVL_TrkIsoVL_Ele8_CaloIdT_CaloIsoVL_TrkIdVL_TrkIsoVL");
     passed_HLT_Ele17_CaloIdT_TrkIdVL_CaloIsoVL_TrkIsoVL_Ele8_CaloIdT_TrkIdVL_CaloIsoVL_TrkIsoVL_ = (isMC_) ? true : this->PassedHLT( jentry, "HLT_Ele17_CaloIdT_TrkIdVL_CaloIsoVL_TrkIsoVL_Ele8_CaloIdT_TrkIdVL_CaloIsoVL_TrkIsoVL");
  
     // muEG:
     passed_HLT_Mu17_Ele8_CaloIdL_ = (isMC_) ? true : this->PassedHLT( jentry, "HLT_Mu17_Ele8_CaloIdL" );
     passed_HLT_Mu8_Ele17_CaloIdL_ = (isMC_) ? true : this->PassedHLT( jentry, "HLT_Mu8_Ele17_CaloIdL" );
     passed_HLT_Mu8_Ele17_CaloIdT_CaloIsoVL_ = (isMC_) ? true : this->PassedHLT( jentry, "HLT_Mu8_Ele17_CaloIdT_CaloIsoVL" );
...
*/
     ...
     if( event_==DEBUG_EVENTNUMBER || DEBUG_EVENTNUMBER==-999 ) {
     ...
     for( unsigned int iEle=0; (iEle<nEle); ++iEle ) {
       ...
       if( event_==DEBUG_EVENTNUMBER || DEBUG_EVENTNUMBER==-999 ) std::cout << "Is good electron (2012)." << std::endl;
...
//     }
       ...
       if( event_==DEBUG_EVENTNUMBER || DEBUG_EVENTNUMBER==-999 ) std::cout << "Passed additional eleID cuts (HLT)." << std::endl;
       ...
       if( event_==DEBUG_EVENTNUMBER || DEBUG_EVENTNUMBER==-999 ) std::cout << "Not matched to any muon." << std::endl;
...
*/
     ...
     for( unsigned int iJet=0; iJet<nAK5PFPUcorrJet; ++iJet ) {
       ...
       if(TMath::Abs(thisJet.Eta()) > 4.7) goodetajet = 0;  

       if(TMath::Abs(thisJet.Eta()) < 2.5) {
         if(thisJet.betaStar > 0.2 * log( nvertex_ - 0.67 ) ) goodetajet = 0;
         if(rmsCandsHandAK5PFPUcorrJet[iJet] > 0.06) goodetajet = 0;
       } else if(TMath::Abs(thisJet.Eta()) < 2.75){
         if(thisJet.betaStar > 0.3 * log( nvertex_ - 0.67 ) ) goodetajet = 0;
         if(rmsCandsHandAK5PFPUcorrJet[iJet] > 0.06) goodetajet = 0;	     
       } else if(TMath::Abs(thisJet.Eta()) < 3.){
         if(rmsCandsHandAK5PFPUcorrJet[iJet] > 0.05) goodetajet = 0;
       } else {
         if(rmsCandsHandAK5PFPUcorrJet[iJet] > 0.055) goodetajet = 0;
       }

       //if( goodetajet==0 ) continue;
       
       
       if (!isLooseJetMva(thisJet.Pt(),thisJet.Eta(),jetIdMvaPhilV1AK5PFPUcorrJet[iJet])) continue;



       // match to genjet:
       float bestDeltaR_genJet=999.;
       ...
       for( unsigned iGenJet=0; iGenJet<nAK5GenJet; ++iGenJet ) {
         ...
         if( thisGenJet.DeltaR(thisJet) < bestDeltaR_genJet ) {
           bestDeltaR_genJet=thisGenJet.DeltaR(thisJet);
       ...
       for( unsigned iPart=0; iPart<nMc; ++iPart ) {
         ...
         if( thisPart.DeltaR(thisJet) < bestDeltaR_part ) {
           ...
           pdgIdPartMom=idMc[mothMc[iPart]];
         }
       }
   
       if( bestDeltaR_part<0.3 ) {
         ...
         thisJet.pdgIdPartMom   = pdgIdPartMom;

       } else {

         if( bestDeltaR_genJet>0.3 ) { //PU
           ...
           thisJet.phiPart = 0.;
           thisJet.ePart   = 0.;
           thisJet.pdgIdPart   = -999;
           thisJet.pdgIdPartMom  =  -999;
     ...
     for( unsigned iJet=0; iJet<leadJets.size() && nJets_<50; ++iJet ) {
       ...
       pdgIdPartMomJet_[nJets_] = leadJets[iJet].pdgIdPartMom;
